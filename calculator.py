...
for arg in sys.argv:
    if "--type=" in arg:
        mode = arg.replace("--type=", "")
    if "--payment=" in arg:
        annuity_payment = float(arg.replace("--payment=", ""))
    if "--principal=" in arg:
        loan_principal = float(arg.replace("--principal=", ""))
    if "--periods=" in arg:
        number_of_payments = int(arg.replace("--periods=", ""))
    if "--interest=" in arg:
        loan_interest = float(arg.replace("--interest=", "")) / (12 * 100)
   ...
   loan_interest is not None and loan_interest < 0):
    ...
    if mode == "diff":
        ...
        if loan_principal is None:
            ...
            number_of_payments = math.log((annuity_payment /
